...
| `code` | Recurso | Onde ocorre |
|--------|---------|-------------|
| `CONTACT_LIMIT_REACHED` | Contatos | `POST /api/v1/leads` ao criar um contato novo |
| `DEAL_LIMIT_REACHED` | Negócios | `POST /api/v1/deals` e `POST /api/v1/leads` quando o payload abre um card |
| `LIST_LIMIT_REACHED` | Listas de contatos | `POST /api/v1/lists` |
| `STORAGE_LIMIT_REACHED` | Storage de mídia | Upload de anexos — o `error` traz o consumo atual e o teto em GB |

Consulte os tetos de cada plano em [Planos e Limites](/plans/limits).
